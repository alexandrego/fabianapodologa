...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-blue-100 py-10">
  <div class="max-w-md w-full bg-white rounded-xl shadow-xl p-8 border border-blue-100">
    <h1 class="text-2xl font-bold text-blue-900 mb-6">Entrar no sistema</h1>
    <form method="POST" action="{{ route('patient.login') }}" x-data="{ loading: false }" @submit="loading = true">
      @csrf
      <div class="mb-4">
        <label for="email" class="block text-sm font-semibold text-blue-800 mb-1">E-mail</label>
        <input type="email" name="email" id="email" required autofocus class="w-full px-4 py-2 rounded-lg border border-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-400">
      </div>
      <div class="mb-6">
      ...
      <div class="mt-4 text-center space-y-2">
        <a href="{{ route('patient.register') }}" class="text-blue-700 underline hover:text-blue-900 text-sm">Cadastrar paciente</a><br>
